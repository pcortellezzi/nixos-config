...
3. Use `pdf-reader` MCP for technical manuals or design documents in PDF.
4. Use `google-drive` MCP for shared assets or requirements.
5. Use `memory-plus` for complex knowledge graph relationship tracking.
6. Use webfetch for raw page content (never guess APIs).
7. Use context7 MCP for library docs.
8. Use grep_app MCP for code examples from GitHub.

### SKILLS SYSTEM
1. Built-in skills in `~/.config/opencode/skills/` — auto-triggered by name
2. **OpenSkills** — use `npx openskills read <skill>` to load marketplace skills
3. Skill directories scanned: `~/.config/opencode/skills/`, `~/.claude/skills/`, `~/.agents/skills/`
4. `<available_skills>` XML in AGENTS.md managed by `npx openskills sync`
...
### SKILL DETECTION
1. When user asks a question or assigns a task, first check available skills in skill directories.
2. If a matching skill exists, read it and follow its instructions.
3. If no matching skill found locally, run `npx openskills read <skill-name>` to find and load a relevant skill.
...
Use `skill(name="<skill-name>")` to load any skill from the ecosystem.
Available skill paths: `~/.config/opencode/skills/`, `~/.claude/skills/`, `~/.agents/skills/`
Marketplace: `npx openskills read <name>`
